...
def format_commit_message(message: str, commit_type: str = "feat") -> str:
    """
    Convert a plain commit message into Conventional Commits format.
    """
...
def commit_changes(repo: Repo, message: str, commit_type: str = "feat") -> Optional[str]:
...
def push_branch(
    ...
    try:
        ...
        if "authentication" in error_text or "permission" in error_text or "403" in error_text:
            raise RuntimeError(
                "GitHub authentication failed. Please check your GitHub token or login credentials."
            ) from exc
